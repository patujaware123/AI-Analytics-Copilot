...
# Uploaded file = use uploaded dataset
# No upload = automatically use default dataset from GitHub
...
if uploaded_file is not None:
    ...
    try:
        ...
        st.info(
            "📊 Default sample dataset loaded automatically. "
            "You can upload your own CSV or Excel file above."
        )
...
sales_lower, sales_upper = iqr_bounds(
    analysis_df["Sales"]
)

profit_lower, profit_upper = iqr_bounds(
    analysis_df["Profit"]
)
...
sales_anomalies = analysis_df[
    analysis_df["Sales_Anomaly"]
].copy()

profit_anomalies = analysis_df[
    analysis_df["Profit_Anomaly"]
].copy()

all_anomalies = analysis_df[
    analysis_df["Any_Anomaly"]
].copy()
...
forecast_data = analysis_df.copy()
...
forecast_available = len(monthly_sales_forecast) >= 3
...
"""
        ...
        # =================================================
            ...
            ("highest" in question
             or "most profitable" in question
             or "top" in question)
...
"""
        ...
        # =================================================
            ...
            "quantity sold" in question
            or "total quantity" in question
        ):
...
"""
        ...
        # =================================================
            ...
            if len(items) >= 2:
                ...
                st.warning(
                    "⚠️ Please mention two matching categories, "
                    "regions, or products to compare."
                )
            ...
            with st.spinner(
                ...
                try:
                    ...
                    if forecast_available:
                        ...
                        forecast_records = "Not available"
...
st.caption(
    ...
    "Cleaning metrics above show the result after "
